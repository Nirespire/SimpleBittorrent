...
public class Util {
	...
	// Given file and destination where splits should go, splits file into 10KB
	// chunks
	public static int splitFile(File file, String rootDir) {
		ArrayList<FileChunk> chunks = new ArrayList<FileChunk>();
		try {
			chunks = getFileChunks(new RandomAccessFile(file, "r"),
					file.getName());
	...
	public static ArrayList<FileChunk> getFileChunks(RandomAccessFile raf,
			String fileName) throws IOException, ChordException {
	...
	// Naming scheme is
	// <rootDir>\<originalFileName>#<currentChunkNum>.<numberOfChunks>
	public static void writeFileChunksToFiles(String rootDir,
			ArrayList<FileChunk> chunks) throws IOException {
		for (FileChunk chunk : chunks) {
			FileOutputStream fos = new FileOutputStream(rootDir + "\\"
					+ chunk.getChunkFilename());
	...
	public static File rebuildFileFromFileChunks(FileChunk[] chunks,
			String newFileName) throws IOException, ChordException {
		return rebuildFileFromFileChunks(
				new ArrayList<FileChunk>(Arrays.asList(chunks)), newFileName);
	}

	// Provided an ArrayList of FileChunk objects, reconstructs the original
	// file.
	// If all parts are not provided,
	public static File rebuildFileFromFileChunks(ArrayList<FileChunk> chunks,
			String newFileName) throws IOException, ChordException {
		if (chunks.isEmpty()) {
			return null;
		}
		...
		if (orderedChunks.size() != numChunks) {
			throw new ChordException(
					"rebuildFileFromChunks: complete set of FileChunks not provided");
		}

		File f = (newFileName == null ? new File("img\\"
				+ orderedChunks.get(0).getFileName()) : new File("img\\"
				+ newFileName));
